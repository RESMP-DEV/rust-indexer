...
use crate::vectordb::milvus_id_for_chunk_id;
...
fn hit_key(chunk: &CodeChunk) -> String {
    if !chunk.id.is_empty() {
        // Milvus stores i64 ids derived from the chunk id; canonicalize so a
        // semantic hit and its lexical twin merge instead of duplicating.
        let canonical_id = chunk
            .id
            .parse::<i64>()
            .map(|id| id.to_string())
            .unwrap_or_else(|_| milvus_id_for_chunk_id(&chunk.id).to_string());
