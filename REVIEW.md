# Review guidance for RESMP-DEV/rust-indexer
...
- Review RESMP-DEV/rust-indexer as the source of truth for Local-first CLI for hybrid code search (BM25 + semantic), index-compatible with rust_sindexer.
...
- Keep this repository a CLI, not an MCP server or daemon. Lexical-only operation must work with zero configuration and --json output must remain stable for scripted callers.
- Do not couple it to rust_sindexer. Preserve typed bincode persistence and lock cache-environment mutations in parallel tests.
