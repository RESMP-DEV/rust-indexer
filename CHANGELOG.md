...
- Review hardening (PR #1): the env-file loader strips both quote styles and
  ...
  switch refuses `index` (forced or not) and `update`, since rebuilding
  into the wrong backend would advance the shared manifest while the
  recorded backend's collection survives to authenticate it; re-homing
  goes through `clear` with the recorded backend configured, so a
